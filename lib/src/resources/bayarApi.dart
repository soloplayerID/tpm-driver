import 'package:TPMDriver/src/response/bayarCheck.dart';
import 'package:TPMDriver/src/response/bayarGet.dart';
import 'package:TPMDriver/src/response/bayarPost.dart';
import 'package:TPMDriver/src/response/cekPembayaran.dart';
import 'package:TPMDriver/src/response/cekStatus.dart';
import 'package:TPMDriver/src/response/hargaGetResponse.dart';
...
class BayarApi {
  ...
  Future<String> bayarCancel(String orderId) async {
    ...
    final response = await _client
        .post("${Paths.BASEURL}${Paths.ENDPOINT_BAYARCANCEL}?id=$orderId");
    if (response.statusCode == 200) {
      Map<String, dynamic> res = jsonDecode(response.body);
      if (res['success']) {
